let fCat='all',fPrio='all',calOff=0,calView='week',dragId=null,editingId=null;
let activeView='focus',selectedNoteId=null,notesCatFilter='all';
let completionChart=null;

const esc=s=>{const d=document.createElement('div');d.textContent=s;return d.innerHTML};
const PO={critique:0,haute:1,moyenne:2,moderee:3,basse:4};
const PL={critique:'Critique',haute:'Haute',moyenne:'Moyenne',moderee:'Modérée',basse:'Basse'};
const PN={critique:5,haute:4,moyenne:3,moderee:2,basse:1};
const CL={travail:'Travail',personnel:'Personnel',sante:'Santé',urgent:'Urgent',apprentissage:'Apprentissage'};
const NCL={travail:'Travail',personnel:'Personnel',idees:'Idées',projets:'Projets',autre:'Autre'};
const EL={do:'Faire maintenant',plan:'Planifier',delegate:'Déléguer',eliminate:'Éliminer'};
const JOURS=['Lun','Mar','Mer','Jeu','Ven','Sam','Dim'];
const MOIS=['jan.','fév.','mars','avr.','mai','juin','juil.','août','sept.','oct.','nov.','déc.'];
const JOURS_FULL=['Dimanche','Lundi','Mardi','Mercredi','Jeudi','Vendredi','Samedi'];
function dk(d){return d.toISOString().slice(0,10)}
function filt(){return tasks.filter(t=>(fCat==='all'||t.cat===fCat)&&(fPrio==='all'||t.prio===fPrio))}
...
document.getElementById('sb-cats').addEventListener('click',e=>{const b=e.target.closest('.sb-cat');if(!b)return;fCat=b.dataset.c;document.querySelectorAll('.sb-cat').forEach(x=>x.classList.remove('active'));b.classList.add('active');renderAll()});
document.getElementById('sb-prios').addEventListener('click',e=>{const b=e.target.closest('.sb-prio');if(!b)return;fPrio=b.dataset.p;document.querySelectorAll('.sb-prio').forEach(x=>x.classList.remove('active'));b.classList.add('active');renderAll()});
...
function fabAction(){
  ...
  else if(activeView==='calendar'){openModal();setTimeout(()=>{const dates=calView==='day'?dayDate(calOff):weekDates(calOff);document.getElementById('f-due').value=dk(dates[0])},50)}
  else openModal();
}
...
function calcNextDue(t){
  const now=new Date();
  if(t.recurrence==='daily'){const d=new Date(now);d.setDate(d.getDate()+1);return dk(d)}
  if(t.recurrence==='weekly'){const day=t.recurDay||1;let d=new Date(now);d.setDate(d.getDate()+1);while(d.getDay()!==day)d.setDate(d.getDate()+1);return dk(d)}
  if(t.recurrence==='monthly'){const day=t.recurDay||1;let d=new Date(now.getFullYear(),now.getMonth(),day);if(d<=now)d.setMonth(d.getMonth()+1);return dk(d)}
  return '';
}
...
async function toggleDone(id){
  ...
  if(t.status!=='done'){
    t.status='done';t.completedAt=dk(new Date());
...
/* ═══ KANBAN ═══ */
function cardHTML(t){
  const dots=Array(PN[t.prio]||1).fill('<span></span>').join('');
  ...
  return `<div class="card" draggable="true" data-id="${t.id}" ondragstart="dStart(event)" ondragend="dEnd(event)">
    ...
    <div class="card-top"><span class="badge b-${t.cat}">${CL[t.cat]||t.cat}</span><span class="card-dots">${dots}</span>${recurIcon}</div>
    <div class="card-title">${esc(t.title)}</div>
    ${t.desc?'<div class="card-desc">'+esc(t.desc)+'</div>':''}
    <div class="card-foot">${t.dur?'<span>&#128339; '+esc(t.dur)+'</span>':''}${t.due?'<span>&#128197; '+t.due+'</span>':''}</div></div>`}
function renderKanban(){
  const f=filt();['todo','doing','done'].forEach(s=>{
    const items=f.filter(t=>t.status===s).sort((a,b)=>PO[a.prio]-PO[b.prio]);
...
function kOver(e){e.preventDefault();e.currentTarget.classList.add('dragover')}
function kLeave(e){e.currentTarget.classList.remove('dragover')}
...
function renderFocus(){
  const now=new Date(),h=now.getHours(),today=dk(now);
  ...
  const scored=pending.map(t=>{let s=(5-(PO[t.prio]||2))*10;if(t.due){const d=Math.ceil((new Date(t.due)-now)/864e5);if(d<=0)s+=100;else if(d<=1)s+=80;else if(d<=3)s+=50;else if(d<=7)s+=20}if(t.eq==='do')s+=30;return{...t,_s:s}}).sort((a,b)=>b._s-a._s).slice(0,3);
  const top3=scored.map((t,i)=>{const d=t.status==='done';const meta=[];if(t.due){const dd=Math.ceil((new Date(t.due)-now)/864e5);meta.push(dd<=0?'<span style="color:var(--red)">En retard</span>':dd<=1?'<span style="color:var(--red)">Demain</span>':'&#128197; '+t.due)}meta.push(PL[t.prio]);
    return `<div class="focus-task${d?' done':''}"><div class="focus-task-rank">${i+1}</div><div class="focus-chk${d?' on':''}" onclick="toggleDone('${t.id}')"></div><div class="focus-task-info"><div class="focus-task-title">${esc(t.title)}</div><div class="focus-task-meta">${meta.join(' · ')}</div></div></div>`}).join('')||'<div class="focus-empty">Aucune tâche en attente !</div>';
  const habitPct=habits.length?Math.round(habitsToday/habits.length*100):0;
  const habitsHTML=habits.map(hb=>{const checked=hb.checks&&hb.checks[today];let streak=0;const dd=new Date();while(hb.checks&&hb.checks[dk(dd)]){streak++;dd.setDate(dd.getDate()-1)}
    return `<div class="focus-habit${checked?' checked':''}" onclick="toggleFocusHabit('${hb.id}')"><div class="focus-chk${checked?' on':''}"></div><span class="focus-habit-name">${esc(hb.name)}</span>${streak>0?'<span class="focus-habit-streak">&#128293; '+streak+'j</span>':''}</div>`}).join('')||'<div class="focus-empty">Aucune habitude</div>';
...
async function toggleFocusHabit(id){const hb=habits.find(x=>x.id===id);if(!hb)return;const today=dk(new Date());if(!hb.checks)hb.checks={};hb.checks[today]=!hb.checks[today];if(!hb.checks[today])delete hb.checks[today];renderAll();await updateHabit(hb)}
...
function renderCalendar(){
  const isDay=calView==='day',dates=isDay?dayDate(calOff):weekDates(calOff),today=dk(new Date());
...
function cdOver(e){e.preventDefault();e.currentTarget.classList.add('dragover')}
function cdLeave(e){e.currentTarget.classList.remove('dragover')}
...
function cuOver(e){e.preventDefault();e.currentTarget.classList.add('dragover')}
function cuLeave(e){e.currentTarget.classList.remove('dragover')}
...
function renderNotes(){
  ...
  document.getElementById('notes-list').innerHTML=filtered.map(n=>`<div class="note-item${selectedNoteId===n.id?' active':''}" onclick="selectNote('${n.id}')"><div class="note-item-title">${esc(n.title||'Sans titre')}</div><div class="note-item-preview">${esc((n.content||'').slice(0,80))}</div><div class="note-item-meta"><span class="badge b-${n.cat}" style="font-size:.55rem">${NCL[n.cat]||n.cat}</span></div></div>`).join('')||'<div style="color:var(--tx3);padding:20px;text-align:center;font-size:.84rem">Aucune note</div>';
}
...
function calcStreaks(h){
  let cur=0;const d=new Date();while(h.checks&&h.checks[dk(d)]){cur++;d.setDate(d.getDate()-1)}
...
function renderHabits(){
  ...
  const html=habits.map(hb=>{
    ...
    const dots=Array.from({length:14},(_,i)=>{const dd=new Date(today);dd.setDate(today.getDate()-13+i);const k=dk(dd);const on=hb.checks&&hb.checks[k]?'on':'';const label=dd.getDate()+'/'+String(dd.getMonth()+1).padStart(2,'0');return`<div class="habit-card-dot ${on}" onclick="toggleHabitDay('${hb.id}','${k}')" title="${label}">${dd.getDate()}</div>`}).join('');
    return `<div class="habit-card"><div class="habit-card-header"><span class="habit-card-name">${esc(hb.name)}</span><button class="habit-card-del" onclick="delHabit('${hb.id}')">&#10005;</button></div><div class="habit-card-dots">${dots}</div><div class="habit-card-streak"><span>&#128293; Série actuelle : <strong>${cur}j</strong></span><span>&#127942; Record : <strong>${best}j</strong></span></div></div>`}).join('')||'<div class="focus-empty">Aucune habitude</div>';
...
function renderStats(){
  const now=new Date(),today=dk(now);
  ...
  let thisW=0,lastW=0;
  for(let i=0;i<=dow;i++){const d=new Date();d.setDate(d.getDate()-i);thisW+=tasks.filter(t=>t.completedAt===dk(d)).length}
  for(let i=0;i<=dow;i++){const d=new Date();d.setDate(d.getDate()-7-i);lastW+=tasks.filter(t=>t.completedAt===dk(d)).length}
  ...
  const catBars=cats.map(c=>{const n=tasks.filter(t=>t.cat===c).length;return`<div class="st-bar-r"><span class="lb">${CL[c]}</span><div class="bar"><div class="fill" style="width:${all?n/all*100:0}%;background:${catColors[c]}"></div></div><span class="vl">${n}</span></div>`}).join('');
  ...
  const prioBars=prios.map(p=>{const n=tasks.filter(t=>t.prio===p).length;return`<div class="st-bar-r"><span class="lb">${PL[p]}</span><div class="bar"><div class="fill" style="width:${all?n/all*100:0}%;background:${prioColors[p]}"></div></div><span class="vl">${n}</span></div>`}).join('');
  // Habit streaks
  const streakHTML=habits.map(hb=>{const{cur,best}=calcStreaks(hb);return`<div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid var(--brd)"><span style="flex:1;font-size:.84rem">${esc(hb.name)}</span><span style="font-size:.82rem;color:var(--or);font-weight:600">&#128293; ${cur}j</span><span style="font-size:.75rem;color:var(--tx3)">Record : ${best}j</span></div>`}).join('')||'<div style="color:var(--tx3);font-size:.82rem">Aucune habitude</div>';
  ...
  // Chart.js
  const labels=[],values=[];
  for(let i=29;i>=0;i--){const d=new Date();d.setDate(d.getDate()-i);labels.push(d.getDate()+' '+MOIS[d.getMonth()]);values.push(tasks.filter(t=>t.completedAt===dk(d)).length)}
...
function checkDeadlines(){
  ...
  const now=new Date(),today=dk(now);
  const tomorrow=new Date(now);tomorrow.setDate(tomorrow.getDate()+1);const tmrw=dk(tomorrow);
  tasks.forEach(t=>{
